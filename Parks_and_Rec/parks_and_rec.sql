...
-- Parameters: variables that are passed as an input into a store procedure and they allow the store procedure to accept an input value
DELIMITER $$ 
CREATE PROCEDURE large_salaries4(employee_idd INT)
BEGIN
	SELECT salary
	FROM employee_salary
    WHERE employee_id = employee_idd
    ;
END $$
DELIMITER ;
